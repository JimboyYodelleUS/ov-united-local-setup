...
class SaleOrder(models.Model):
    ...
    amount_untaxed = fields.Monetary(
        string="Untaxed Amount", store=True, compute="_amount_all", tracking=5
    )
    ...
    amount_total = fields.Monetary(
        string="Total", store=True, compute="_amount_all", tracking=4
    )
    ...
    tag_ids = fields.Many2many(
        "crm.tag", "sale_order_tag_rel", "order_id", "tag_id", string="Tags"
    )
    ...
    def _compute_currency_rate(self):
        for order in self:
            if not order.company_id:
                order.currency_rate = (
                    order.currency_id.with_context(date=order.date_order).rate or 1.0
                )
                continue
            elif (
                order.company_id.currency_id and order.currency_id
    ...
    def onchange_partner_id(self):
        ...
        if not self.env.context.get("not_self_saleperson") or not self.team_id:
            default_team = (
                self.env.context.get("default_team_id", False)
                or self.partner_id.team_id.id
            )
            values["team_id"] = (
                self.env["crm.team"]
    ...
    def _compute_field_value(self, field):
        if field.name == "invoice_status" and not self.env.context.get(
            "mail_activity_automation_skip"
        ):
            filtered_self = self.filtered(
                lambda so: (so.user_id or so.partner_id.user_id)
        ...
        upselling_orders = filtered_self.filtered(
            lambda so: so.invoice_status == "upselling"
        )
    ...
    def _prepare_invoice(self):
        ...
        invoice_vals = {
            ...
            "fiscal_position_id": (
                self.fiscal_position_id
                or self.fiscal_position_id.get_fiscal_position(
                    self.partner_invoice_id.id
                )
            ).id,
            "partner_bank_id": self.company_id.partner_id.bank_ids.filtered(
                lambda bank: bank.company_id.id in (self.company_id.id, False)
            )[:1].id,
    ...
    def _action_confirm(self):
        ...
        for order in self:
            if any(
                expense_policy not in [False, "no"]
